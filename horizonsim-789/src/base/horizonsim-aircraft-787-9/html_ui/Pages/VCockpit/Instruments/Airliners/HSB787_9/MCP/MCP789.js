var b78xmcp = (function (exports) {
    'use strict';

    /** VS/FPA MCP Window */
    class B78xMCPHdgTrkWindow {
        /**
        ...
        onUpdate() {
            if (this.isPowered) {
                const trkActive = this.isTrkActive();
                this.mode = trkActive ? 'TRK' : 'HDG';
         ...
         */
        isTrkActive() {
            return SimVar.GetSimVarValue('L:XMLVAR_TRK_MODE_ACTIVE', 'boolean') > 0;
        }
        /** @inheritdoc */
        get valueText() {
            return this.value;
    ...
    class B78xMCPVsFpaWindow {
        ...
        onUpdate() {
            if (this.isPowered) {
                const fpaActive = this.isFpaActive();
                this.mode = fpaActive ? 'FPA' : 'V/S';
                if (this.isActive()) {
         ...
         */
        isFpaActive() {
            return SimVar.GetSimVarValue('L:XMLVAR_FPA_MODE_ACTIVE', 'boolean') > 0;
        }
        /** @inheritdoc */
        get valueText() {
            return this.value;
        ...
        connectedCallback() {
            ...
            switch (this.windowType) {
                case McpWindowTypes.HdgTrk:
                    this.window = new B78xMCPHdgTrkWindow(this);
                    break;
                case McpWindowTypes.IasMach:
                    this.window = new B78xMCPSpdWindow(this);
                    break;
                case McpWindowTypes.VsFpa:
                    this.window = new B78xMCPVsFpaWindow(this);
                    // Asobo hack to fix V/S window model scale
                    this.classList.add('b787-10-vspeed-element');
                    break;
            }
    ...
    registerInstrument('wt-b787-mcp', HSB789_MCP);

    exports.HSB789_MCP = HSB789_MCP;
